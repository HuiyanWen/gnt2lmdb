...
int main(int argc, char* argv[]){
	cout << "Loading..." << endl;
	if (argc < 4) {
		//gflags::ShowUsageWithFlagsRestrict(argv[0], "gnt2lmdb");	
		argv[1] = "./lexicon3755.txt";
		argv[2] = "./hwtst.gnt";
		argv[3] = "./3755_test_RGB2/";
	}
	...
	//long *start = new long[lines.size() + 1];
	//cout << "yes" << endl;
	...
	while (_ftelli64(fpIn) < fpLen){
		...
		char tagcode[3];
		tagcode[2] = '\0';
		//memcpy(&gntlen, buff, len_all);
		memcpy(tagcode, buff + len_all, len_label);
		//memcpy(&width, buff + len_label + len_all, len_width);
		//memcpy(&height, buff + len_label + len_all + len_width, len_height);
		//cout << tagcode << endl;
		int position;
	...
	//cout << start[lines.size() - 1] << endl;
	//cout << "countflag:" << count_flag << endl;
	_fseeki64(fpIn, 0, SEEK_SET);
	...
	while (pos_ptr<fpLen)//遍历new_gnt
	{
		...
		str.resize(buff2.size());
		memcpy(&str[0], buff2.data(), buff2.size());*/
		if (FLAGS_gray == FALSE)
...
//	std::vector<std::pair<std::string, int> > lines;
//
//	std::ifstream infile("L:/RGB_resnet3755/lexicon3755.txt");
//	
//	std::string line;
//	string *line_ch = new string[3755];
//	size_t pos;
